...
export default function CardList(props) {
    return (
        <div>
            {props.items.map((item) => <ExtendedCard key={item.id} simple={props.simple} title={item.title || item.username} datePublished={item.dateTimeRegistered} imageUrl={item.imageURL}
